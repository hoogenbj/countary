...
public class DatabaseDataModel implements DataModel {
    ...
    @Override
    public Budget cloneBudget(Budget budget, String newName) throws SQLException {
    ...
    private BudgetItem getBudgetItem(long budgetItemId) throws SQLException {
        BudgetItem bf = null;
        String query = "select f.id, f.name, bf.note, bf.planned, f.kind, bf.id, bf.budgetId, f.categoryId, " +
                "(select count(*)>0 from allocation a where a.budgetItemId=bf.id) from item f " +
                "join budget_item bf on bf.itemId = f.id where bf.id = ?";
        try (Connection connection = DriverManager.getConnection(settings.getDatabaseUrl(), connectionProperties);
             PreparedStatement statement = connection.prepareStatement(query)) {
            statement.setLong(1, budgetItemId);
            try (ResultSet resultSet = statement.executeQuery()) {
                if (resultSet.next()) {
                    Item item = new Item(resultSet.getLong(1), resultSet.getString(2),
                            Kind.valueOf(resultSet.getString(5)), getCategory(connection, resultSet.getLong(8)));
                    Set<Tag> tags = getTagsForItem(item);
                    bf = new BudgetItem(resultSet.getLong(6), getBudget(resultSet.getLong(7)),
                            item, resultSet.getString(3),
                            resultSet.getBigDecimal(4), tags, !resultSet.getBoolean(9));
                }
            }
        }
